...
- **Classes.** *Marksman* is the baseline. *Cannoneer* unlocks by reaching wave 4: slow, long-range splashing shells, a recoil hop that jumps AWAY from the cursor, and a heat mechanic where shells fired without moving in between drag out the next windup until you move 40 units to vent. *Skirmisher* unlocks at 200 career kills: short range (260), very fast throwing blades, 90 HP. A blade released within half a second of moving is a **Tempo** shot (+40 % damage, +15 % crit), so the class rewards the step-throw-step cadence the Cannoneer punishes you for skipping. E is a blade dash that cuts every enemy it passes through once, and the class-only Riposte augment refunds dash cooldown per cut. *Arbalest* (unlocks at wave 2) carries a heavy crossbow: one slow quarrel that flies straight and punches through up to four enemies, losing 15 % per body; firing spends the crank and only walking rewinds it (90 units), so the tempo is aim, fire, run. *Rifleman* (wave 3) carries an M1 Garand: eight hard-hitting semi-automatic rounds, then the empty clip ejects with a ping and 1.6 s of reload you spend moving; R ejects a partial clip early. *Gunslinger* (150 career kills) carries twin Berettas: short reach, quick feet, and every attack also fires the off hand at the nearest other enemy in reach for 60 % damage; thirty rounds, then a 1.1 s reload (R reloads early). Each weapon has its own report, muzzle animation and ammunition readout. See `src/game/heroes.ts`.
...
- **Feel.** The run settings offer two feels. *Iron* (default) is the intended game: 60 % reach, slow deliberate shots, fast enemy fire with short warnings, weaker augments, a draft every second wave and a rarer Prismatic. *Easy* exposes a **Pressure** slider: 0 % is a gentle floor (full speed, 90 % reach, slower and softer enemies with longer warnings, a draft every wave, a firmer hit-stun shove) and every step slides all knobs together toward Iron, stopping at 90 % so Easy never silently becomes Iron. Both are presets over the same authored numbers (`src/game/tuning.ts`); the arena header names anything other than Iron.
